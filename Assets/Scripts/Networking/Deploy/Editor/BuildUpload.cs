...
namespace ConnectedGames.Build
{
    public class BuildUpload
    {
        ...
        public BuildUpload(string label, string path, string clientApi, string platform, string projectId, string orgId, string projectName,
            string accessToken) : this(label, path, clientApi, platform)
        {
        ...
        {
            Debug.Log("HandleEnd: " + m_Request.downloadHandler.text + " err?:" + m_Request.isNetworkError + " code: " + m_Request.responseCode +
                      " desired: " + desiredCode);
        ...
        {
            string targetUrl = k_CloudArtifactsUrl + m_ProjectName + "/buildtargets/_local/builds/" + m_BuildId.ToString() +
                               "/artifacts/.ZIP file/upload/" + m_UploadedFile.Name;
