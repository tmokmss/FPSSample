    ...
    where TPackageInfo : PackageInfo, new()
{
    ...
    protected int ProcessPackageHeader(byte[] packageData, int packageSize, out NetworkMessage content, out byte[] assembledData,
        out int assembledSize, out int headerSize)
    {
        counters.packagesIn++;
        ...
        else
        {
            foreach (var eventInfo in info.events)
            {
                ...
                if (eventInfo.reliable)
                {
                    ...
                    GameDebug.Log(
                        "Resending lost reliable event: " + ((GameNetworkEvents.EventType) eventInfo.type.typeId) + ":" + eventInfo.sequence);
