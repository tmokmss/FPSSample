...
public class LightProbesVolumeSettings : MonoBehaviour
{
    ...
    public void Populate()
    {
        ...
        for (int z = 0; z < zCount; z++)
        {
            for (int x = 0; x < xCount; x++)
            {
                ...
                foreach (var hit in hits)
                {
                    ...
                    EditorUtility.DisplayProgressBar("Tracing floor collisions", (z * x).ToString() + "/" + (zCount * xCount).ToString(),
                        (float) (z * x) / (float) (zCount * xCount));
                }
            }
        }
        ...
        if (discardInsideGeometry)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("Checking probes inside geometry", j.ToString() + "/" + VertPositions.Count,
                    (float) j / (float) VertPositions.Count);
                ...
                hitsForward = Physics.RaycastAll(forwardRay, Vector3.Distance(positionCandidate, insideTestPosition), -1,
                    QueryTriggerInteraction.Ignore);
                hitsBackward = Physics.RaycastAll(backwardRay, Vector3.Distance(positionCandidate, insideTestPosition), -1,
                    QueryTriggerInteraction.Ignore);
