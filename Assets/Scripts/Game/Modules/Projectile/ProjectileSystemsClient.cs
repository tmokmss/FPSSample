...
public class HandleProjectileSpawn : BaseComponentSystem
{
    ...
    protected override void OnCreateManager()
    {
        ...
        PredictedProjectileGroup =
            GetComponentGroup(typeof(ProjectileData), typeof(PredictedProjectile), ComponentType.Subtractive<DespawningEntity>());
...
//            var inProjectileEntity = inProjectile.GetComponent<GameObjectEntity>().Entity;
            ...
            if (!m_world.GetEntityManager().HasComponent<PredictedProjectile>(inProjectileEntity))
            {
                for (var i = 0; i < predictedProjectileEntities.Length; i++)
                {
                    ...
                    if (ProjectileModuleClient.logInfo.IntValue > 0)
                        GameDebug.Log("ProjectileSystemClient. Predicted projectile" + predictedProjectileEntity + " matched with " +
                                      inProjectileEntity + " from server. startTick:" +
...
//            m_world.RequestDespawn(gameObject, PostUpdateCommands);
            ...
            if (ProjectileModuleClient.logInfo.IntValue > 0)
                GameDebug.Log(string.Format("<color=red>Predicted projectile {0} destroyed as it was not verified. startTick:{1}]</color>", entity,
                    predictedEntity.startTick));
        }
    }
}
