...
public class AnimGraph_Banking : AnimGraphAsset
{
    ...
    class Instance : IAnimGraphInstance, IGraphLogic
    {
        ...
        public void UpdateGraphLogic(GameTime time, float deltaTime)
        {
            ...
            if (animState.charLocoState == CharacterPredictedData.LocoState.GroundMove)
            {
                var movement = new Vector3(animState.position.x, 0f, animState.position.z) -
                               new Vector3(m_PreviousPosition.x, 0f, m_PreviousPosition.z);
                ...
                delta = Mathf.Clamp(delta, -m_Settings.bankingSettings.maxBankContribution * deltaTime,
                    m_Settings.bankingSettings.maxBankContribution * deltaTime);

                // TODO: (sunek) Make it be multiplied by velocity
                animState.banking = Mathf.Clamp(animState.banking + delta, -m_Settings.bankingSettings.bankMagnitude,
                    m_Settings.bankingSettings.bankMagnitude);
