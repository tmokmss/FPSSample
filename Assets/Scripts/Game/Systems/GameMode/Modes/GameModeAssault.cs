...
public class GameModeAssault : IGameMode
{
        ...
        switch (m_Phase)
        {
            case Phase.PreGame:
                if (m_GameModeSystemServer.GetGameTimer() == 0)
                {
                    ...
                    else
                    {
                        ...
                        var l = StringFormatter.Write(ref _msgBuf, 0, "Match started! {0} is attacking!",
                            m_GameModeSystemServer.teams[s_AttackTeam].name);
            ...
            case Phase.Active:
                ...
                if (m_ActiveCapturePoint != null)
                {
                    ...
                    {
                        var l = StringFormatter.Write(ref _msgBuf, 0, "{0} captured {1}. {2}/{3}", m_GameModeSystemServer.teams[s_AttackTeam].name,
                            m_ActiveCapturePoint.objectiveName, m_NumCaptured + 1, m_NumCapturePoints);
                        m_GameModeSystemServer.chatSystem.SendChatAnnouncement(new CharBufView(_msgBuf, l));
                        SelectNextCapturePoint();
                        if (m_ActiveCapturePoint == null)
                        {
    ...
    void UpdateCurrentCapturePoint()
    {
        ...
        for (int i = 0; i < players.Length; i++)
        {
            ...
            bool switchOk = false;
            if (player.teamIndex == s_DefendTeam && m_DefendersBasePoint != null &&
                InsideCylinder(position, defendersBasePoint, m_DefendersBasePoint.height, m_DefendersBasePoint.radius))
                switchOk = true;

            if (player.teamIndex == s_AttackTeam && m_AttackersBasePoint != null &&
                InsideCylinder(position, attackersBasePoint, m_AttackersBasePoint.height, m_AttackersBasePoint.radius))
                switchOk = true;
        ...
        else
        {
            if (attacking > 0)
            {
                ...
                m_ActiveCapturePoint.captured = Mathf.Clamp01(m_ActiveCapturePoint.captured +
                                                              m_World.worldTime.tickInterval * attackMultiplier / m_ActiveCapturePoint.captureTime);
            ...
            else if (defending > 0)
            {
                ...
                m_ActiveCapturePoint.captured = Mathf.Clamp01(m_ActiveCapturePoint.captured -
                                                              m_World.worldTime.tickInterval * defendMultiplier / m_ActiveCapturePoint.captureTime);
        ...
        {
            GameDebug.Log(string.Format("Capture Point {0} switched from {1} to {2}", m_ActiveCapturePoint.name,
                m_ActiveCapturePoint.status.ToString(), status.ToString()));
