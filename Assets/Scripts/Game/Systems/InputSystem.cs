...
public class InputSystem
{
    ...
    public void AccumulateInput(ref UserCommand command, float deltaTime)
    {
        ...
        deltaMousePos += deltaTime * (new Vector2(
                             ((Game.Input.GetKey(KeyCode.Keypad4) ? -1.0f : 0.0f) + (Game.Input.GetKey(KeyCode.Keypad6) ? 1.0f : 0.0f)) *
                             s_JoystickLookSensitivity.x,
                             -invertY * Game.Input.GetAxisRaw("RightStickY") * s_JoystickLookSensitivity.y));
        ...
        command.buttons.Or(UserCommand.Button.PrimaryFire,
            (Game.Input.GetMouseButton(0) && Game.GetMousePointerLock()) || (Game.Input.GetAxisRaw("Trigger") < -0.5f));
